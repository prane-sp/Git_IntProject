trigger updatecontactrolecount on Opportunity (before insert, before update)
{
    ...
    for (List<OpportunityContactRole> oppcntctrle3 : [select OpportunityId from OpportunityContactRole where (OpportunityContactRole.Role != '' and OpportunityContactRole.OpportunityId in :oppty_con.keySet())])//Query for role values
    {    
        if (oppcntctrle3.Size() > 0)
        {
            roleCount = oppcntctrle3.Size();     
        }
    }
    ...
    for (Opportunity Oppty : system.trigger.new) //Check if  roles exist in the map or contact role isn't required 
    {
        ...
        Oppty.NumberofContactRoleswithRole__c = roleCount;
    }
}